...
const OwnerSetting = () => {
  const [existingOwners, setExistingOwners] = useState<{ ownerName: string; ethAddress: string }[]>(
    [],
  )
  const schema = object({
    owner: z
  ...
  return (
    <div className='flex flex-col justify-center items-center w-full h-full box-border'>
      <div>
        <span className='text-textWhite text-2xl font-bold'>② Owner setting</span>
          ...
          <StepWrapper>
            ...
            <div className='rounded-2xl bg-bgDarkMid  box-border py-4 px-8'>
              <div className='flex flex-col text-textWhite text-base '>
                <span>
                  This Meson wallet on Ethereum has {existingOwners.length} owners. Optional:
                  Provide a name for each owner.
                </span>
              </div>
              <Spacer size={16} axis={'vertical'} />
              <div className='flex flex-col'>
                {existingOwners.map((owner: any, index: number) => (
                  <div key={owner.ethAddress} className='mb-2'>
                    <div className='grid grid-cols-8 gap-2 w-full'>
                      ...
                      <div className='col-span-6'>
                        <EthAddress ethAddress={owner.ethAddress} size={4.5} length={'full'} />
                      </div>
                    </div>
                  </div>
                ))}
