...
const SwitchSignerDetail: React.FC<SwitchSignerDetailType> = ({ onClose }) => {
  ...
  const trezorAccounts = useSelector<RootState, ITrezorState>((state) => state.trezorWallet)
  const { signerWalletAddress, serializedPath, balance, isConnected, wallet } = useSelector<
    RootState,
    SignerState
  >((state) => state.signerWallet)

  const [trezorFullAccounts, setTrezorAccountsWithBalance] = useState<NewTrezorAccountType[]>([])
  ...
  return (
    <div className='flex flex-col text-textWhite'>
              ...
              trezorFullAccounts.map((account: any) => (
                <div
                  className={`grid grid-cols-2 gap-x-8 box-border rounded-xl hover:bg-dark w-full p-2 ${
                    account.address == primarySigner.signerWalletAddress ? 'bg-dark' : ''
                  }`}
                  key={account.address}
      ...
      <div className='flex flex-row justify-around'>
        ...
        <Button
          btnVariant={
            primarySigner.signerWalletAddress == signerWalletAddress ? 'disable' : 'primary'
          }
          btnSize={'lg'}
          btnType={'submit'}
